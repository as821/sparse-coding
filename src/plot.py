...
def plot_patch_recon(args, basis, x, z):
    ...
    x = rearrange(x, "d (a b c) -> d a b c", a=args.patch_sz, b=args.patch_sz, c=3)
    normalized = np.zeros_like(x)
    for i in range(3):  # For each color channel
        channel = x[:, :,:,i]
        normalized[:, :,:,i] = (channel - channel.min()) / (channel.max() - channel.min())
        
    patch_sz = (z != 0).to(int).sum(axis=1)
    arr = np.argsort(patch_sz)


    # don't visualize zero codes
    idx = 0
    while idx < arr.shape[0] and patch_sz[arr[idx]] == 0:
        idx += 1
    if idx >= arr.shape[0]:
        print("All patches have zero codes, nothing to visualize.")
        return None
    ...
    for i in range(k):
        ...
        for i in range(3):  # For each color channel
            channel = recon[:, :,i]
            recon[:, :, i] = (channel - channel.min()) / (channel.max() - channel.min())
        
        axes.flat[base].imshow(recon)

        # original patch
        axes.flat[base + 1].imshow(normalized[arr[idx]])
